...
- pgdir：页目录的基址。之前几次实验知道了CPU通过satp寄存器找到当前页表的根节点，从而进行地址翻译。pgdir字段保存的就是**每个进程的页表页表根目录的物理地址**。当进行进程切换时，内核需要将下一个要运行进程的 `pgdir` 值加载到 `satp` 寄存器中，这样才能正确地切换到新的地址空间。（页目录：存放指向页表的指针的结构）
...
- list_entry_t proc_list：所有PCB的双向线性列表，proc_struct中的成员变量**list_link**将链接入这个链表中。为了遍历。
...
​	该部分负责从一个已经运行的进程（父进程）复制或共享资源，创建出一个新的子进程（或内核线程），这样的话新进程就能快速继承父进程已有的环境，不用从零一个一个创造。

在代码中负责完成以下工作：

| 步骤 | 操作                   | 目的                                                         |
| ---- | ---------------------- | ------------------------------------------------------------ |
| 1️⃣    | 分配一个新的 PCB       | 创建空壳的 `proc_struct`                                     |
| 2️⃣    | 为其分配内核栈         | 让新进程在内核态有独立栈空间，以便进行系统调用和处理内核级任务，确保新进程在执行时不会干扰父进程的内核栈。 |
| 3️⃣    | 复制或共享内存空间     | 根据标志 clone/share 父进程的内存管理结构                    |
| 4️⃣    | 复制中断帧和上下文     | 让子进程从正确的位置继续执行，通常涉及到CPU寄存器的保存和恢复 |
| 5️⃣    | 分配唯一的 PID         | 区分不同进程                                                 |
| 6️⃣    | 插入全局哈希表与进程表 | 加入系统调度可识别列表                                       |
| 7️⃣    | 设置状态为 RUNNABLE    | 标记可以被调度运行“RUNNABLE态”                               |
| 8️⃣    | 返回子进程的 PID       | 父进程获得新 PID                                             |
...
```c++
    ...
    copy_mm(clone_flags,proc); //根据clone_flags判断是复制父进程的内存（独立副本）；还是共享父进程的内存。
...
##### 3、proc_run()-切换CPU执行权，实现并发

​	该部分实现了“从当前运行进程切换到另一个进程”的真正动作，也就是CPU层面的“**上下文切换**”。在整个调度系统中，schedule决定下一个执行的线程是谁，然后调用proc_run()进行线程的切换。

​	**上下文（context）**包含：

- 所有CPU寄存器的状态（如ra、sp、通用寄存器）
- 程序计数器（pc）
- 栈指针
- 页表基地址（base）
- 进程调度状态信息
...
​	首先先声明一下proc_run(struct proc_struct *proc)中表示要让这个proc进程成为“当前将要运行的进程（current）”。

```c++
void proc_run(struct proc_struct *proc)
{
    if (proc != current) { // 只在切换不同进程时才操作
        bool intr_flag;
        local_intr_save(intr_flag);      // 1、 关中断，防止切换时被打断
        {
            struct proc_struct *prev = current;  
            current = proc;              // 2、 记录新进程为当前
           

            // 3、上下文切换（核心）
            switch_to(&(prev->context), &(current->context));

            //  一旦 switch_to 执行返回，代表新的进程已“回切”回来
        }
        local_intr_restore(intr_flag);   // 4、打开中断
    }
}
```
...
题干：请说明proc_struct中struct context context和struct trapframe *tf成员变量含义和在本实验中的作用是啥？

回答：

##### 1、成员变量含义：

- proc_struct中的struct context context保存的是CPU部分寄存器（如ra、sp、s0~s11）状态，用来实现“内核线程”之间的上下文切换。
- struct trapframe *tf 指向进程内核栈顶的陷入帧，保存了当CPU从用户态或线程启动时的完整寄存器状态（包括通用寄存器、程序计数器 `epc`、状态寄存器等）。



##### 2、在本实验中的作用：

- proc_struct中的context context 用于switch_to（）函数在不同进程间实现上下文切换，每当进程被切换出去，其context会保存当前的状态（CPU的部分寄存器状态），以便下次恢复。
- struct trapframe *tf：当线程第一次运行或从中断返回时，内核根据 tf恢复CPU状态，确保线程从正确位置继续执行。在代码中体现在do_fork()中，子进程第一次运行时，从tf中恢复寄存器状态，作为新线程执行的起点。
