...
static int
copy_to_user_pages(pde_t *pgdir, uintptr_t la, const void *src, size_t len)
{
    while (len > 0)
    {
        uintptr_t page_base = ROUNDDOWN(la, PGSIZE);
        size_t offset = la - page_base;
        size_t n = PGSIZE - offset;
        if (n > len)
        {
            n = len;
        }
        struct Page *page = get_page(pgdir, page_base, NULL);
        if (page == NULL)
        {
            return -E_NO_MEM;
        }
        memcpy((char *)page2kva(page) + offset, src, n);
        la += n;
...
​        uintptr_t la_end = ROUNDUP(end, PGSIZE);
​        if ((ret = mm_map(mm, la, la_end - la, vm_flags, NULL)) != 0)
​        {
​            goto bad_pgdir;
​        }

​        size_t file_bytes = ph.p_filesz + (start - la);
​        off_t file_offset = ph.p_offset - (off_t)(start - la);

​        for (uintptr_t addr = la; addr < la_end; addr += PGSIZE)
​        {
...
​            if (file_bytes > 0)
​            {
​                size_t tocopy = (file_bytes > PGSIZE) ? PGSIZE : file_bytes;
​                if ((ret = load_icode_read(fd, page2kva(page), tocopy, file_offset)) != 0)
​                {
​                    goto bad_pgdir;
​                }
​                file_bytes -= tocopy;
...
​    uintptr_t stack_bot = stack_top - USTACKSIZE;
​    if ((ret = mm_map(mm, stack_bot, USTACKSIZE, VM_READ | VM_WRITE | VM_STACK, NULL)) != 0)
​    {
​        goto bad_pgdir;
​    }
​    for (uintptr_t addr = stack_bot; addr < stack_top; addr += PGSIZE)
​    {
​        struct Page *page = pgdir_alloc_page(mm->pgdir, addr, PTE_U | PTE_R | PTE_W);
​        if (page == NULL)
​        {
​            ret = -E_NO_MEM;
​            goto bad_pgdir;
...
​    uintptr_t uargv_store[EXEC_MAX_ARG_NUM];
​    for (int i = argc - 1; i >= 0; i--)
​    {
​        size_t len = strlen(kargv[i]) + 1;
​        sp -= len;
​        sp &= ~(uintptr_t)0x3;
​        if ((ret = copy_to_user_pages(mm->pgdir, sp, kargv[i], len)) != 0)
​        {
​            goto bad_pgdir;
​        }
​        uargv_store[i] = sp;
...
​    uintptr_t argv_store = sp;
​    for (int i = 0; i < argc; i++)
​    {
​        if ((ret = copy_to_user_pages(mm->pgdir, argv_store + i * sizeof(uintptr_t), &uargv_store[i], sizeof(uintptr_t))) != 0)
​        {
​            goto bad_pgdir;
​        }
​    }
​    uintptr_t null_ptr = 0;
​    if ((ret = copy_to_user_pages(mm->pgdir, argv_store + argc * sizeof(uintptr_t), &null_ptr, sizeof(uintptr_t))) != 0)
​    {
​        goto bad_pgdir;
​    }

​    sp -= sizeof(uintptr_t);
​    if ((ret = copy_to_user_pages(mm->pgdir, sp, &argv_store, sizeof(uintptr_t))) != 0)
​    {
​        goto bad_pgdir;
​    }
​    sp -= sizeof(uintptr_t);
​    if ((ret = copy_to_user_pages(mm->pgdir, sp, &argc, sizeof(uintptr_t))) != 0)
​    {
​        goto bad_pgdir;
​    }
